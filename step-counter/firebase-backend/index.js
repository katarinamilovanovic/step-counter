...
app.post('/api/updateHealthData', async (req, res) => {
  ...
  try {
    ...
    await db.collection('healthData').doc(uid).set({
      uid: uid, date: date, steps: steps, stressLevel: stressLevel, waterIntake: waterIntake, exercise: exercise, generalFeel: generalFeel
    });

    res.status(200).send({ message: 'Health data updated successfully!' });
  } catch (error) {
    console.error('Error updating health data:', error);
    res.status(500).send({ error: 'Error updating health data: ' + error.message });
  }
});
